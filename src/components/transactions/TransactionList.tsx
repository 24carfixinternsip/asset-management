...
import { Eye, RotateCcw, User, Building2, Clock, CheckCircle2 } from "lucide-react";
import { format } from "date-fns";
import { th } from "date-fns/locale";
import { Transaction } from "@/hooks/useTransactions";

// ส่วนย่อย: Card สำหรับมือถือ
const TransactionCard = ({ tx, showActions, onView, onReturn }: { tx: Transaction; showActions?: boolean; onView: (tx: Transaction) => void; onReturn?: (tx: Transaction) => void; }) => {
  ...
  return (
    <div className="bg-card p-4 rounded-lg border shadow-sm flex flex-col gap-3 relative overflow-hidden">
      <div className="flex justify-between items-start gap-2">
         <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1">
               ...
               {!showActions && <StatusBadge status={tx.status} className="h-5 text-[10px]" />}
            </div>
      ...
      <div className="grid grid-cols-2 gap-2 mt-1">
         ...
         {showActions && onReturn && <Button variant="default" size="sm" onClick={() => onReturn(tx)} className="w-full h-9"><RotateCcw className="h-4 w-4 mr-1" /> รับคืน</Button>}
      </div>
    </div>
  );
};
...
export function TransactionList({ data, isLoading, variant, onView, onReturn }: TransactionListProps) {
  const formatDate = (d: string) => format(new Date(d), 'd MMM yy HH:mm', { locale: th });
  ...
  return (
    <>
          ...
          <Table>
            <TableHeader className="sticky top-0 bg-card z-10 shadow-sm">
              <TableRow>
                ...
                {variant === 'history' && <TableHead className="w-[100px]">สถานะ</TableHead>}
            ...
            <TableBody>
              {data.map((tx) => (
                <TableRow key={tx.id}>
                  ...
                  {variant === 'history' && <TableCell><StatusBadge status={tx.status} /></TableCell>}
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                       <Button size="sm" variant="ghost" onClick={() => onView(tx)}><Eye className="h-4 w-4" /></Button>
                       {variant === 'active' && onReturn && <Button size="sm" variant="outline" onClick={() => onReturn(tx)} className="text-primary hover:bg-primary/5 gap-1"><RotateCcw className="h-3.5 w-3.5" /> คืน</Button>}
                    </div>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <div className="md:hidden p-4 space-y-3">
        {data.map((tx) => (
           <TransactionCard key={tx.id} tx={tx} showActions={variant === 'active'} onView={onView} onReturn={onReturn} />
        ))}
      </div>
    </>
  );
